<?php
namespace HR\JobBundle\Controller;
...
class DeliveryController extends Controller
{
    ...
    public function newAction(Request $request, $jobId)
    {
        ...
        $this->get('breadcrumb')
            ->add($job->getPosition(), $this->generateUrl('job_show', array('jobId' => $job->getId())))
            ->add('发送简历');
...
//            return $this->redirect($this->generateUrl('job_show', array('jobId' => $jobId)));
        ...
        $delivery->setTitle(sprintf('%s 应聘 %s 职位', $user->getScreenName(), $job->getPosition()));
        ...
        if ($form->isValid()) {
            ...
            $this->get('hr.mailer')->sendResumeEmailMessage($delivery);
    ...
    /**
     * @return \HR\JobBundle\EntityManager\DeliveryManager
     */
